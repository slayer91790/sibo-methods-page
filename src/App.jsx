...
// ---------- Firebase config ----------
// Set your Firebase web config in Netlify env as REACT_APP_* vars.
// This code also supports an injected __firebase_config string (optional).
let firebaseConfig;
...
const isFirebaseConfigValid = () => {
  return firebaseConfig && Object.values(firebaseConfig).every((v) => v && String(v).trim() !== '');
};
...
const EvidenceTierExplanation = () => {
  const tiersData = [
    { tier: 1, title: 'Tier 1: Strong Evidence', description: 'Backed by higher-quality scientific studies, such as randomized controlled trials.' },
    { tier: 2, title: 'Tier 2: Promising Evidence', description: 'Supported by pilot or smaller studies; promising but needs more research.' },
    { tier: 3, title: 'Tier 3: Anecdotal / Case Report', description: 'Primarily based on user experiences or case reports.' },
    { tier: 0, title: 'Caution: No Evidence / Potential Harm', description: 'No supportive evidence for SIBO and/or potential harm.' },
  ];
  return (
...
const AiPatternAnalysis = () => {
  ...
  return (
    <section className="max-w-4xl mx-auto mt-16 p-6 bg-white rounded-xl shadow-md border border-gray-200">
      <h2 className="text-2xl font-bold text-gray-800 text-center mb-6">What we see across success stories</h2>
...
// ---------- Sample data (replace with your full list when ready) ----------
const siboMethodsData = [
  {
    id: 1,
    title: 'Elemental Diet (short term)',
    summary: 'A nutritionally complete liquid formula used short-term under clinical guidance. Often used when other approaches are not tolerated.',
    evidenceTier: 1,
    citation: { text: 'Discuss with a clinician; research exists on short-course elemental diets for SIBO-like symptoms.', url: '' },
    ...
    protocol: [
      { phase: 'Phase 1 — Preparation', steps: [{ title: 'Overview', description: 'Set expectations, plan duration with clinician.' }] },
      { phase: 'Phase 2 — Active', steps: [{ title: 'Follow schedule', description: 'Use formula per plan; monitor tolerance.' }] },
      { phase: 'Phase 3 — Reintroduction', steps: [{ title: 'Gradual foods', description: 'Reintroduce foods methodically while tracking symptoms.' }] }
    ]
  },
  {
    id: 2,
    title: 'Herbal Antimicrobials (combo)',
    summary: 'Botanical protocols used in cycles; commonly referenced blends combine multiple herbs.',
    evidenceTier: 2,
    citation: { text: 'Pilot/smaller studies exist; quality varies. Use with professional guidance.', url: '' },
    ...
    protocol: [
      { phase: 'Phase 1 — Foundation', steps: [{ title: 'Dietary baseline', description: 'Simplify triggers; ensure hydration.' }] },
      { phase: 'Phase 2 — Active', steps: [{ title: 'Cycle antimicrobials', description: 'Follow cycle plan with breaks; track symptoms.' }] },
      { phase: 'Phase 3 — Maintenance', steps: [{ title: 'Reassess', description: 'Evaluate changes; discuss next steps with clinician.' }] }
    ]
  },
  {
    id: 3,
    title: 'Antibiotics (clinician-directed)',
    summary: 'Prescription options may be used based on testing and clinical assessment.',
    evidenceTier: 1,
    citation: { text: 'Talk to a licensed clinician. Evidence differs by presentation and testing.', url: '' },
    ...
    protocol: [
      { phase: 'Phase 1 — Evaluation', steps: [{ title: 'Testing & plan', description: 'Clinician evaluation; shared decision-making.' }] },
      { phase: 'Phase 2 — Treatment', steps: [{ title: 'Adherence', description: 'Take medicine exactly as prescribed.' }] },
      { phase: 'Phase 3 — Follow-up', steps: [{ title: 'Review', description: 'Assess response and next steps with clinician.' }] }
    ]
  },
  {
    id: 4,
    title: 'Prokinetic & Meal Spacing',
    summary: 'Focuses on gut motility support and spacing meals to support the MMC.',
    evidenceTier: 2,
    citation: { text: 'Physiology-informed strategies; evidence varies by agent.', url: '' },
    ...
    protocol: [
      { phase: 'Phase 1 — Baseline', steps: [{ title: 'Spacing', description: 'Aim for meal spacing to support MMC (as tolerated).' }] },
      { phase: 'Phase 2 — Support', steps: [{ title: 'Prokinetic', description: 'Discuss options with clinician if appropriate.' }] },
      { phase: 'Phase 3 — Review', steps: [{ title: 'Reassess', description: 'Fine-tune with professional support if needed.' }] }
    ]
  },
  {
    id: 5,
    title: 'Low-FODMAP (short-term symptom management)',
    summary: 'Structured elimination/reintroduction to identify triggers and manage symptoms short term.',
    evidenceTier: 2,
    citation: { text: 'Research for IBS symptom management exists; adapt with clinician for SIBO context.', url: '' },
    commonSymptoms: ['Bloating', 'Gas', 'Abdominal pain'],
    sampleDay: {
      title: 'Example day (illustrative)',
      schedule: [
    ...
    protocol: [
      ...
      { phase: 'Phase 3 — Personalization', steps: [{ title: 'Sustain', description: 'Build a long-term, diverse diet based on findings.' }] }
    ]
  },
  {
    id: 6,
    title: 'Stress, Sleep, and Movement Support',
    summary: 'Lifestyle pillars that often influence GI symptoms: stress reduction, sleep quality, and gentle movement.',
    evidenceTier: 3,
    ...
    protocol: [
      ...
      { phase: 'Phase 2 — Build', steps: [{ title: 'Small changes', description: 'Add gentle, realistic daily habits.' }] },
...
function Header({ user, onGoHome, onSubmitMethod, onFeedback }) {
  ...
  const doGoogleSignIn = async () => {
    ...
    try { await signInWithPopup(auth, googleProvider); }
      ...
      try { await signInAnonymously(auth); } catch (err) { console.error('Anon sign-in failed:', err); }
  ...
  return (
    <header className="sticky top-0 z-10 bg-white border-b border-gray-200">
      <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between">
        <button onClick={onGoHome} className="text-xl font-extrabold tracking-tight">SIBO Recovery</button>
        <nav className="flex items-center gap-3">
          <button onClick={onSubmitMethod} className="px-3 py-1.5 rounded-md bg-indigo-600 text-white text-sm">Submit</button>
          <button onClick={onFeedback} className="px-3 py-1.5 rounded-md bg-purple-600 text-white text-sm">Feedback</button>
          {user ? (
            <button disabled={busy} onClick={doSignOut} className="px-3 py-1.5 rounded-md border text-sm">Sign out</button>
          ) : (
            <button disabled={busy} onClick={doGoogleSignIn} className="px-3 py-1.5 rounded-md border text-sm">Sign in</button>
          )}
        </nav>
      </div>
    </header>
...
const MethodCard = ({ method, onSelect, onVote, votes, userVote }) => (
  ...
  >
    <div className="p-6">
      <div className="mb-3"><EvidenceTierBadge tier={method.evidenceTier} /></div>
...
const MethodListPage = ({ methods, onSelectMethod, onVote, votes, userVotes, onSortChange, onOpenAdvisor }) => (
  <div className="p-4 sm:p-6 md:p-8">
    <header className="mb-10 text-center">
      <h1 className="text-4xl font-extrabold tracking-tight text-gray-900 sm:text-5xl">Community-Sourced SIBO Protocols</h1>
      <p className="mx-auto mt-4 max-w-3xl text-lg text-gray-600">Explore recovery methods backed by community experience and general evidence summaries. Vote on what you\'ve tried and see what has worked for others.</p>
      <button onClick={onOpenAdvisor} className="mt-6 transform rounded-lg bg-gradient-to-r from-purple-500 to-indigo-600 px-6 py-3 font-bold text-white shadow-lg transition-all duration-300 ease-in-out hover:-translate-y-1 hover:shadow-xl">✨ Get Help from AI Protocol Advisor</button>
    </header>
    ...
    <footer className="mt-12 px-4 text-center text-sm text-gray-500">
      <p>This site is for educational purposes only and is not medical advice. Always consult a qualified healthcare professional.</p>
    </footer>
  </div>
);
...
const MethodDetailPage = ({ method, onBack, user }) => (
  <div className="mx-auto max-w-4xl p-4 sm:p-6 md:p-8">
    <button onClick={onBack} className="mb-8 flex items-center font-semibold text-blue-600 hover:text-blue-800">
      <svg xmlns="http://www.w3.org/2000/svg" className="mr-2 h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 19l-7-7 7-7" /></svg>
      Back to All Methods
    </button>
    ...
    <div className="mb-8 rounded-r-lg border-l-4 border-blue-500 bg-blue-50 p-4 text-blue-800">
      ...
      {method.citation.url && (
        <a href={method.citation.url} target="_blank" rel="noopener noreferrer" className="mt-2 inline-block text-sm font-semibold text-blue-600 hover:underline">View Study →</a>
      )}
    </div>
      ...
      <div className="mb-8">
        ...
        <div className="flex flex-wrap gap-2">
          {method.commonSymptoms.map((symptom, i) => (
            <span key={i} className="rounded-full bg-gray-200 px-3 py-1 text-sm font-medium text-gray-700">{symptom}</span>
          ))}
        </div>
      </div>
    )}
    ...
    <div className="space-y-8">
      {method.protocol.map((phase, i) => (
        <div key={i} className="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-md">
          <div className="border-b border-gray-200 bg-gray-100 p-4"><h2 className="text-2xl font-bold text-gray-800">{phase.phase}</h2></div>
...
function CommentsSection({ methodId, user }) {
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      try { await signInAnonymously(auth); } catch (anonError) { console.error('Sign-in failed:', anonError); setError('Failed to sign in. Please try again.'); }
    }
  };
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-md">
        {user ? (
          <form onSubmit={handleSubmit} className="mb-6">
            <textarea className="w-full rounded-lg border border-gray-300 p-3 focus:border-blue-500 focus:ring-2 focus:ring-blue-500" rows="4" placeholder="Share your experience with this method..." value={newComment} onChange={(e) => setNewComment(e.target.value)} />
            <button type="submit" className="mt-3 rounded-lg bg-blue-600 px-4 py-2 font-semibold text-white transition-colors hover:bg-blue-700 disabled:bg-gray-400" disabled={!newComment.trim()}>Post Comment</button>
          </form>
        ) : (
          <div className="mb-6 rounded-lg border-2 border-dashed border-gray-300 p-4 text-center">
            <p className="text-gray-600">Want to share your experience?{' '}<button onClick={handleGoogleSignIn} className="font-semibold text-blue-600 hover:underline">Sign in with Google</button> to join the discussion.</p>
          </div>
        )}
          ...
          {comments.length > 0 ? (
            comments.map((c) => (
              <div key={c.id} className="border-b border-gray-200 pb-4 last:border-b-0">
                ...
                <p className="mb-2 text-xs text-gray-500">{c.timestamp ? new Date(c.timestamp.toDate()).toLocaleString() : 'Just now'}</p>
...
function SubmitMethodPage({ onBack, user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await addDoc(collection(db, 'submissions'), { ...formData, submittedBy: user.uid, submittedAt: serverTimestamp() });
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-4xl p-4 text-center sm:p-6 md:p-8">
        ...
        <p className="mb-8 text-lg text-gray-600">Please{' '}<button onClick={async () => { if (auth && googleProvider) { try { await signInWithPopup(auth, googleProvider); } catch (error) { console.error('Google sign-in failed:', error); } } }} className="font-semibold text-blue-600 hover:underline">sign in with Google</button>{' '}to submit a new method. This helps us keep the submissions genuine.</p>
  ...
  return (
    <div className="mx-auto max-w-4xl p-4 sm:p-6 md:p-8">
      ...
      <p className="mb-8 text-gray-600">Thank you for contributing to the community! Please provide as much detail as possible. Your submission will be reviewed before being published.</p>
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-gray-200 bg-white p-8 shadow-md">
        ...
        <div>
          ...
          <textarea name="protocol" id="protocol" rows="8" required className="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" placeholder="Describe the phases and steps in detail. Include timing and duration." value={formData.protocol} onChange={handleChange} />
        </div>
        <div>
        ...
        <div>
          <button type="submit" disabled={isSubmitting} className="w-full justify-center rounded-md bg-green-600 py-2 px-4 text-sm font-medium text-white shadow-sm transition-colors hover:bg-green-700 focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2 disabled:bg-gray-400">{isSubmitting ? 'Submitting...' : 'Submit for Review'}</button>
        </div>
      </form>
    </div>
  );
...
function FeedbackPage({ onBack, user }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsSubmitting(true);
    try {
      await addDoc(collection(db, 'feedback'), { feedbackText: feedback, submittedBy: user.uid, submittedAt: serverTimestamp() });
  ...
  if (!user) {
    return (
      <div className="mx-auto max-w-4xl p-4 text-center sm:p-6 md:p-8">
        ...
        <p className="mb-8 text-lg text-gray-600">Please{' '}<button onClick={async () => { if (auth && googleProvider) { try { await signInWithPopup(auth, googleProvider); } catch (error) { console.error('Google sign-in failed:', error); } } }} className="font-semibold text-blue-600 hover:underline">sign in with Google</button>{' '}to submit feedback.</p>
        <button onClick={onBack} className="font-semibold text-blue-600 hover:text-blue-800">Back to All Methods</button>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-4xl p-4 sm:p-6 md:p-8">
      ...
      <p className="mb-8 text-gray-600">Have an idea to improve the site? Found a bug? Let us know! Your feedback helps make this a better resource for the community.</p>
      <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-gray-200 bg-white p-8 shadow-md">
        <div>
        ...
        <div>
          <button type="submit" disabled={isSubmitting} className="w-full justify-center rounded-md bg-purple-600 py-2 px-4 text-sm font-medium text-white shadow-sm transition-colors hover:bg-purple-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:ring-offset-2 disabled:bg-gray-400">{isSubmitting ? 'Submitting...' : 'Submit Feedback'}</button>
        </div>
      </form>
    </div>
  );
...
function GeminiAdvisor({ methods, onClose }) {
  ...
  const getAdvice = async () => {
    if (selectedSymptoms.length === 0) { setAdvice('Please select at least one symptom to get advice.'); return; }
    setIsLoading(true);
    setAdvice('');
    ...
    if (!GEMINI_API_KEY) { setAdvice('Gemini API key is not configured. Set REACT_APP_GEMINI_API_KEY in your environment variables.'); setIsLoading(false); return; }

    try {
      const response = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-1.5-flash:generateContent?key=${GEMINI_API_KEY}`, {
        method: 'POST', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ systemInstruction: { role: 'system', parts: [{ text: systemPrompt }] }, contents: [{ role: 'user', parts: [{ text: userQuery }] }] }),
      });
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="max-h-[90vh] w-full max-w-2xl overflow-y-auto rounded-2xl bg-white p-6 shadow-2xl sm:p-8">
        <div className="mb-6 flex items-center justify-between">
          ...
          <button onClick={onClose} className="text-gray-500 hover:text-gray-800"><svg className="h-8 w-8" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M6 18L18 6M6 6l12 12" /></svg></button>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {allSymptoms.map((sym) => (
              <button key={sym} onClick={() => toggleSymptom(sym)} className={`rounded-full border-2 px-4 py-2 text-sm font-semibold transition-colors ${selectedSymptoms.includes(sym) ? 'border-indigo-600 bg-indigo-600 text-white' : 'border-gray-300 bg-white text-gray-700 hover:bg-gray-100'}`}>{sym}</button>
            ))}
          </div>
        </div>

        <button onClick={getAdvice} disabled={isLoading || selectedSymptoms.length === 0} className="w-full rounded-lg bg-indigo-600 py-3 px-6 font-bold text-white shadow-md transition-colors hover:bg-indigo-700 disabled:cursor-not-allowed disabled:bg-gray-400">{isLoading ? 'Analyzing...' : 'Get AI Advice'}</button>
...
export default function App() {
  ...
  useEffect(() => {
    if (user && db) {
      ...
      const unsub = onSnapshot(userVotesCollection, (snapshot) => {
        const mine = {}; snapshot.docs.forEach((d) => { mine[d.id] = d.data().vote; }); setUserVotes(mine);
      });
      return () => unsub();
    } else { setUserVotes({}); }
  }, [user]);
  ...
  const handleVote = async (id, voteType) => {
    ...
    if (!currentUser) {
      try { const provider = new GoogleAuthProvider(); const result = await signInWithPopup(auth, provider); currentUser = result.user; }
      catch (e) { console.error('Sign-in for vote failed', e); return; }
    }
    ...
    try {
      await runTransaction(db, async (tx) => {
        const [voteDocSnap, userVoteSnap] = await Promise.all([tx.get(voteDocRef), tx.get(userVoteDocRef)]);
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50 p-8">
        <div className="rounded-lg bg-white p-10 text-center shadow-md">
          ...
          <p className="text-gray-700">Firebase configuration is missing. If you're the site owner, set environment variables in your hosting provider.</p>
        </div>
      </div>
    );
  }
  ...
  if (!authReady) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-gray-50"><p className="text-gray-600">Loading Community Hub...</p></div>
    );
  }
  ...
  const renderPage = () => {
    switch (currentPage) {
      case 'detail':
        if (!selectedMethod) { handleBack(); return null; }
  ...
  return (
    <main className="min-h-screen font-sans bg-gray-50">
      ...
      {isAdvisorOpen && <GeminiAdvisor methods={methods} onClose={() => setIsAdvisorOpen(false)} />}
      {renderPage()}
    </main>
  );
}
